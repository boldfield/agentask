...
# queries the registry's v2 API for each distinct one. The registry serves plain HTTP on
# port 32050 (HTTPS fails there), so this deliberately uses http://.
verify-fleet-tags:
	...
	for img in $$images; do \
	  ...
	  rm -f /tmp/verify-fleet-tags.$$$$.err; \
	  if echo "$$resp" | jq -e --arg t "$$tag" '(.tags // []) | any(.[]; . == $$t)' >/dev/null 2>&1; then \
