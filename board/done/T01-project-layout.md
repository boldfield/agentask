...
- `go mod init github.com/boldfield/odonian` (Go 1.22+).
- Directory layout:
  - `cmd/odonian/main.go` — entrypoint, prints version and exits 0 for now.
...
## Acceptance criteria
- `make build` produces `bin/odonian`.
- `make run` prints a version line and exits cleanly.
- `go vet ./...` and `go test ./...` pass (no tests yet is fine).

## Result

**PR:** https://github.com/boldfield/odonian/pull/1
**Branch:** odonian/T01-project-layout
