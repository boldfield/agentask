...
func TestReviewVerdictSubmission(t *testing.T) {
	...
	// Setup: Create project, document, implement task
	proj, err := store.CreateProject(ctx, "test-project", "https://github.com/example/repo")
	if err != nil {
		t.Fatalf("failed to create project: %v", err)
	}
	...
	implementTasks, err := store.CreateTasks(ctx, proj.ID, []TaskInput{
		{
	...
	if err != nil {
		t.Fatalf("failed to create implement task: %v", err)
	}
	implementTask := implementTasks[0]

	// Manually create review tasks (simulating MR-7 spawning behavior)
	reviewTasks, err := store.CreateTasks(ctx, proj.ID, []TaskInput{
		{
			Title:      "Review: Implement feature [opus]",
			Spec:       "Review the implementation",
			DocumentID: doc.ID,
			Model:      "opus",
		},
		{
			Title:      "Review: Implement feature [sonnet]",
			Spec:       "Review the implementation",
			DocumentID: doc.ID,
			Model:      "sonnet",
		},
	...
	// Update the review tasks to set their kind and target_task_id (as MR-7 would do)
	// Also set them to "ready" state since MR-7 would spawn them ready for claim
	conn := store.Conn()
	for _, rt := range reviewTasks {
		_, err := conn.ExecContext(ctx, `
			UPDATE task SET kind='review', target_task_id=?, review_round=0, state='ready'
			WHERE id=?
		`, implementTask.ID, rt.ID)
		if err != nil {
			t.Fatalf("failed to set review task kind: %v", err)
		}
	}

	// Test 1: Submit review task with approve verdict
	reviewTask1 := reviewTasks[0]
	claimed1, err := store.ClaimTask(ctx, reviewTask1.ID, "reviewer1", reviewTask1.Model, 5*time.Minute)
	if err != nil {
		t.Fatalf("failed to claim review task 1: %v", err)
	}

	approve := "approve"
	reviewNote := "Looks good!"
	result1, err := store.SubmitTask(ctx, claimed1.ID, "reviewer1", reviewNote, &approve, nil)
	if err != nil {
		t.Fatalf("failed to submit review task with approve: %v", err)
	}

	if result1.State != "done" {
		t.Errorf("expected review task to be in 'done', got '%s'", result1.State)
	}
	if result1.Verdict == nil || *result1.Verdict != "approve" {
		t.Errorf("expected verdict='approve', got %v", result1.Verdict)
	}
	if result1.Result == nil || *result1.Result != reviewNote {
		t.Errorf("expected result to be review note, got %v", result1.Result)
	}

	// Test 2: Submit review task with reject verdict
	reviewTask2 := reviewTasks[1]
	claimed2, err := store.ClaimTask(ctx, reviewTask2.ID, "reviewer2", reviewTask2.Model, 5*time.Minute)
	if err != nil {
		t.Fatalf("failed to claim review task 2: %v", err)
	}

	reject := "reject"
	rejectNote := "Needs more work"
	result2, err := store.SubmitTask(ctx, claimed2.ID, "reviewer2", rejectNote, &reject, nil)
	if err != nil {
		t.Fatalf("failed to submit review task with reject: %v", err)
	}

	if result2.State != "done" {
		t.Errorf("expected review task to be in 'done', got '%s'", result2.State)
	}
	if result2.Verdict == nil || *result2.Verdict != "reject" {
		t.Errorf("expected verdict='reject', got %v", result2.Verdict)
	}

	// Test 3: Verify verdict events are recorded on parent task
	parentEvents, err := store.ListEvents(ctx, implementTask.ID)
	if err != nil {
		t.Fatalf("failed to list parent task events: %v", err)
	}

	verdictEvents := make([]Event, 0)
	for _, e := range parentEvents {
		if e.Kind == "review" {
			verdictEvents = append(verdictEvents, e)
		}
	}

	if len(verdictEvents) != 2 {
		t.Fatalf("expected 2 verdict events on parent, got %d", len(verdictEvents))
	}

	// Check that both verdicts are recorded
	verdictMap := make(map[string]bool)
	for _, e := range verdictEvents {
		if e.Verdict != nil {
			verdictMap[*e.Verdict] = true
		}
	}
	if !verdictMap["approve"] || !verdictMap["reject"] {
		t.Errorf("expected both approve and reject verdicts in events, got %v", verdictMap)
	}

	// Test 4: Submit implement task with verdict should fail
	implementTask2, err := store.CreateTasks(ctx, proj.ID, []TaskInput{
		{
			Title:      "Another implement task",
			Spec:       "Do more things",
			DocumentID: doc.ID,
			Model:      "haiku",
		},
	...
	if err != nil {
		t.Fatalf("failed to create implement task 2: %v", err)
	}

	// Promote to ready before claiming
	_, err = store.PromoteTask(ctx, implementTask2[0].ID)
	if err != nil {
		t.Fatalf("failed to promote implement task 2: %v", err)
	}

	claimed3, err := store.ClaimTask(ctx, implementTask2[0].ID, "worker2", "haiku", 5*time.Minute)
	if err != nil {
		t.Fatalf("failed to claim implement task 2: %v", err)
	}

	_, err = store.SubmitTask(ctx, claimed3.ID, "worker2", "result", &approve, nil)
	if err == nil {
		t.Error("expected verdict to be forbidden for implement task, but submission succeeded")
	}
	var valErr *ValidationError
	if !errors.As(err, &valErr) {
		t.Errorf("expected ValidationError, got %T", err)
	} else if valErr.Code != "VERDICT_FORBIDDEN" {
		t.Errorf("expected error code VERDICT_FORBIDDEN, got %s", valErr.Code)
	}

	// Test 5: Submit review task without verdict should fail
	reviewTask3, err := store.CreateTasks(ctx, proj.ID, []TaskInput{
		{
			Title:      "Another review task",
			Spec:       "Review more stuff",
			DocumentID: doc.ID,
			Model:      "opus",
		},
	})
	if err != nil {
		t.Fatalf("failed to create review task 3: %v", err)
	}

	// Promote and setup as review task
	_, err = store.PromoteTask(ctx, reviewTask3[0].ID)
	if err != nil {
		t.Fatalf("failed to promote review task 3: %v", err)
	}

	_, err = conn.ExecContext(ctx, `
		UPDATE task SET kind='review', target_task_id=? WHERE id=?
	`, implementTask.ID, reviewTask3[0].ID)
	if err != nil {
		t.Fatalf("failed to set review task kind: %v", err)
	}

	claimed4, err := store.ClaimTask(ctx, reviewTask3[0].ID, "reviewer3", "opus", 5*time.Minute)
	if err != nil {
		t.Fatalf("failed to claim review task 3: %v", err)
	}

	_, err = store.SubmitTask(ctx, claimed4.ID, "reviewer3", "result", nil, nil)
	if err == nil {
		t.Error("expected error when verdict is missing on review task, but submission succeeded")
	}
	if !errors.As(err, &valErr) {
		t.Errorf("expected ValidationError, got %T", err)
	} else if valErr.Code != "MISSING_VERDICT" {
		t.Errorf("expected error code MISSING_VERDICT, got %s", valErr.Code)
	}

	// Test 6: Submit review task with invalid verdict should fail
	reviewTask4, err := store.CreateTasks(ctx, proj.ID, []TaskInput{
		{
			Title:      "Yet another review task",
			Spec:       "Review even more stuff",
			DocumentID: doc.ID,
			Model:      "sonnet",
		},
	})
	if err != nil {
		t.Fatalf("failed to create review task 4: %v", err)
	}

	// Promote and setup as review task
	_, err = store.PromoteTask(ctx, reviewTask4[0].ID)
	if err != nil {
		t.Fatalf("failed to promote review task 4: %v", err)
	}

	_, err = conn.ExecContext(ctx, `
		UPDATE task SET kind='review', target_task_id=? WHERE id=?
	`, implementTask.ID, reviewTask4[0].ID)
	if err != nil {
		t.Fatalf("failed to set review task kind: %v", err)
	}

	claimed5, err := store.ClaimTask(ctx, reviewTask4[0].ID, "reviewer4", "sonnet", 5*time.Minute)
	if err != nil {
		t.Fatalf("failed to claim review task 4: %v", err)
	}

	badVerdict := "maybe"
	_, err = store.SubmitTask(ctx, claimed5.ID, "reviewer4", "result", &badVerdict, nil)
	if err == nil {
		t.Error("expected invalid verdict to be rejected, but submission succeeded")
	}
	if !errors.As(err, &valErr) {
		t.Errorf("expected ValidationError, got %T", err)
	} else if valErr.Code != "INVALID_VERDICT" {
		t.Errorf("expected error code INVALID_VERDICT, got %s", valErr.Code)
	}
}
