-- Rebuild task table to widen state CHECK constraint to include 'abandoned'
...
CREATE TABLE task_new (
  ...
  title TEXT NOT NULL,
  spec TEXT NOT NULL,
  state TEXT NOT NULL CHECK (state IN ('backlog', 'ready', 'in_progress', 'review', 'approved', 'done', 'blocked', 'failed', 'superseded', 'abandoned')),
  assignee TEXT,
  ...
  track TEXT NOT NULL DEFAULT 'build',
...
SELECT id, project_id, document_id, title, spec, state, assignee, lease_expires_at, result, model, kind, review_models, review_round, target_task_id, verdict, agent_merge, held, created_at, updated_at, archived_at, superseded_by, escalate, track
FROM task;
